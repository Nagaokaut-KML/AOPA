...
class Segment_dataloader:
    def __init__(self, mode="train", length=400, rate=0.8):
        """
            ...
            rate : integer
                train,val rate
        """
        f = open("./params/predict_class.yaml", "r+")
        ...
        if mode=="train":
            ...
            # ------ load npz file --------------
            self.load_npz_csv(bath_path, val_npz_list, val_csv_list)
    ...
    def load_npz_csv(self,bath_path, train_npz_list, train_csv_list):
        for order, path in enumerate(tqdm(train_npz_list)):
            self.npz_list.append( np.load(bath_path + path + '.npz')['arr_0'] )

            csv_dict = pd.read_csv(bath_path + train_csv_list[order] , index_col=0 ,names=('path','time')).to_dict(orient='index')
            new_dict = {}
